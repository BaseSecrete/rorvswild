...
RorVsWild.Local.prototype.getSectionNameFilter = function() {
  var input = document.querySelector("[data-rorvswild-local-section-filter]")
  return input ? input.value.trim() : ""
}

RorVsWild.Local.escapeHtml = function(text) {
...
RorVsWild.Local.highlightMatches = function(text, query) {
  if (!query) return RorVsWild.Local.escapeHtml(text)
  ...
  while ((index = lowerText.indexOf(lowerQuery, start)) !== -1) {
    result += RorVsWild.Local.escapeHtml(text.slice(start, index))
    result += '<mark class="rorvswild-local-panel__section-filter-highlight">'
    result += RorVsWild.Local.escapeHtml(text.slice(index, index + query.length))
    result += "</mark>"
    start = index + query.length
  }

  result += RorVsWild.Local.escapeHtml(text.slice(start))
  return result
}

RorVsWild.Local.prototype.clearSectionNameHighlights = function() {
  document.querySelectorAll("[data-rorvswild-local-filter-text]").forEach(function(element) {
    if (element.dataset.rorvswildLocalSectionNameOriginal !== undefined) {
      element.innerHTML = element.dataset.rorvswildLocalSectionNameOriginal
...
RorVsWild.Local.prototype.highlightFilterText = function(query) {
  document.querySelectorAll("[data-rorvswild-local-section]").forEach(function(section) {
    ...
    if (element.dataset.rorvswildLocalSectionNameOriginal === undefined)
      element.dataset.rorvswildLocalSectionNameOriginal = element.innerHTML

    element.innerHTML = RorVsWild.Local.highlightMatches(element.textContent, query)
  })
}

RorVsWild.Local.prototype.applySectionFilters = function() {
  var kind = this.getActiveKindFilter()
  var query = this.getSectionNameFilter()
  var hasNameFilter = query.length > 0
  var toggleButton = document.querySelector("[data-rorvswild-local-toggle-low-impact]")

  document.querySelectorAll("[data-rorvswild-local-section]").forEach(function(section) {
    var name = (section.dataset.rorvswildLocalSectionName || "").toLowerCase()
    var matchesKind = !kind || section.dataset.kind === kind
    var matchesName = !hasNameFilter || name.indexOf(query.toLowerCase()) !== -1
  ...
  if (hasNameFilter) {
    ...
    this.clearSectionNameHighlights()
    if (kind) {
      if (toggleButton) toggleButton.style.display = "none"
    } else {
      this.resetLowImpactSections()
...
RorVsWild.Local.prototype.filterSectionsByName = function(event) {
