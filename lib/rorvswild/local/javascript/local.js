...
RorVsWild.Local.prototype.sections = function() {
  if (!this.currentExecution) return []
  return this.filterSections(this.currentExecution.sections()).map(function(section) {
...
RorVsWild.Local.prototype.sectionsImpactPerKind = function() {
  if (!this.currentExecution) return []
  return this.currentExecution.sectionsImpactPerKind().map(function(item) {
    return {
      kind: item.kind,
      impact: item.impact,
      isActive: item.kind === this.sectionKindFilter
    }
  }.bind(this))
}

RorVsWild.Local.prototype.showLowImpactToggle = function() {
  return !this.isFilteringSections() && this.currentExecution && this.currentExecution.hasLowImpactSections()
}
