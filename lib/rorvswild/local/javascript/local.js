...
RorVsWild.Local.prototype.toggleSection = function(event) {
  if (this.currentExecution) this.currentExecution.toggleSection(event)
}

RorVsWild.Local.prototype.renderSections = function() {
  var container = this.root.querySelector("[data-rorvswild-local-sections-container]")
  if (!container || !this.currentExecution) return

  var active = document.activeElement
  var refocusFilter = active && active.classList.contains("rorvswild-local-panel__section-filter__input")
  var selectionStart = refocusFilter && active.selectionStart
  var selectionEnd = refocusFilter && active.selectionEnd

  var templates = Barber.partials()
  container.innerHTML = Mustache.render(templates["RorVsWild.Local.Sections"], this.currentExecution, templates)
  Barber.listenEvents(container, this)
  Prism.highlightAllUnder(container)

  if (refocusFilter) {
    var input = container.querySelector(".rorvswild-local-panel__section-filter__input")
    if (input) {
      input.focus()
...
RorVsWild.Local.prototype.toggleLowImpactSections = function(event) {
  ...
  this.currentExecution.toggleLowImpactSections()
  this.renderSections()
}

RorVsWild.Local.prototype.filterSectionsByText = function(event) {
  if (!this.currentExecution) return
  this.currentExecution.filterSectionsByText(event)
  this.renderSections()
}

RorVsWild.Local.prototype.filterSectionsByKind = function(event) {
  if (!this.currentExecution) return
  this.currentExecution.filterSectionsByKind(event)
  this.renderSections()
}
...
RorVsWild.Local.prototype.goToRequestDetail = function(event) {
  ...
  this.currentExecution = this.currentRequest = this.requests.find(function(req) { return req.uuid == uuid })
  if (this.currentExecution) {
    this.currentExecution.resetSectionFilters()
    this.currentExecution.loadSections()
  }
...
RorVsWild.Local.prototype.goToJobDetail = function(event) {
  var uuid = event.currentTarget.dataset.uuid
  this.currentExecution = this.currentJob = this.jobs.find(function(job) { return job.uuid == uuid })
  if (this.currentExecution) {
    this.currentExecution.resetSectionFilters()
    this.currentExecution.loadSections()
  }
...
RorVsWild.Local.Request.prototype.resetSectionFilters = function() {
  this.sectionKindFilter = null
  this.sectionQueryFilter = ""
  this.lowImpactExpanded = false
}

RorVsWild.Local.Request.prototype.isFilteringSections = function() {
  return this.sectionKindFilter != null || this.sectionQueryFilter.length > 0
}

RorVsWild.Local.Request.prototype.filterSections = function(sections) {
...
RorVsWild.Local.Request.prototype.visibleSections = function() {
  var query = this.sectionQueryFilter
  return this.filterSections(this.sections).map(function(section) {
    return Object.assign({}, section, {
      locationHtml: query.length > 0 ? RorVsWild.Local.highlightMatches(section.location, query) : Mustache.escape(section.location)
    })
  })
}

RorVsWild.Local.Request.prototype.loadSections = function() {
  var self = this
...
RorVsWild.Local.Request.prototype.showLowImpactToggle = function() {
  return !this.isFilteringSections() && this.sections.some(function(section) { return section.isLowImpact })
}

RorVsWild.Local.Request.prototype.lowImpactToggleLabel = function() {
  return this.lowImpactExpanded ? "↑ Hide low impact sections" : "↓ View low impact sections"
}

RorVsWild.Local.Request.prototype.toggleSection = function(event) {
  ...
  event.currentTarget.closest(".rorvswild-local-panel__details__section").classList.toggle("is-open", section.isOpen)
}

RorVsWild.Local.Request.prototype.toggleLowImpactSections = function() {
  this.lowImpactExpanded = !this.lowImpactExpanded
}

RorVsWild.Local.Request.prototype.filterSectionsByText = function(event) {
  this.sectionQueryFilter = event.currentTarget.value.trim().toLowerCase()
}

RorVsWild.Local.Request.prototype.filterSectionsByKind = function(event) {
