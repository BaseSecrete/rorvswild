...
RorVsWild.Local.prototype.sections = function() {
  ...
  if (!execution) return []
  return this.filterSections(execution.sections()).map(function(section) {
...
RorVsWild.Local.prototype.sectionsImpactPerKind = function() {
  var execution = this.currentExecution()
  if (!execution) return []
  return execution.sectionsImpactPerKind().map(function(item) {
    return {
      kind: item.kind,
      impact: item.impact,
...
RorVsWild.Local.prototype.currentExecution = function() {
  return this.currentRequest || this.currentJob
}
